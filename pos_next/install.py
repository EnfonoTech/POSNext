"""
Installation and Migration hooks for FatehPOS
...
def after_install():
	...
	try:
		log_message("FatehPOS: Running post-install setup", level="info")
		...
		log_message("FatehPOS: Installation completed successfully", level="success")
	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(title="FatehPOS Installation Error", message=frappe.get_traceback())
		log_message(f"FatehPOS: Installation error - {e!s}", level="error")
		raise
...
def after_migrate():
	...
	try:
		...
		log_message("FatehPOS: Migration completed successfully", level="success")
	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(title="FatehPOS Migration Error", message=frappe.get_traceback())
		log_message(f"FatehPOS: Migration error - {str(e)}", level="error")
		raise
...
def reclaim_pos_settings_doctype(quiet=False):
	...
	`invoice_fields` and `pos_search_fields`. FatehPOS ships its own
	non-Single `POS Settings` (module FatehPOS) with per-profile config
	...
	belongs to FatehPOS (module == 'FatehPOS' and not Single), exits
	...
	if row and row.module == "FatehPOS" and not row.issingle:
		if not quiet:
			log_message("POS Settings already owned by FatehPOS, nothing to reclaim", level="info")
		return
	...
	if not after or after.module != "FatehPOS" or after.issingle:
		frappe.log_error(
			title="POS Settings Reclaim Error",
			message=(
